...
class contasreceberimoveisController extends DefaultController{
	...
	function parcelas(){
		if(!$_GET['id']){
			...
			$this->redirect('contasreceberimoveis/listar');
		}
    ...
    public function excluirParcela(){
        $this->carregarModulo('parcela');
        $this->parcela->excluir($_GET['id'], false);
        die;
    }
	...
	function pagarParcela(){
		...
		$this->view->form = Form::create($this->pagamento->mapeador(), 'contasreceberimoveis/pagaraction');
	}
	...
	function pagaraction(){
        ...
        $this->carregarSubModulo('contasreceberimoveis','baixa');
        $this->carregarSubModulo('contasreceberimoveis','pagamento');
        $this->carregarSubModulo('contasreceberimoveis','parcela');
        $this->carregarSubModulo('contasreceberimoveis','contasreceberimoveis');


        $parcela = $this->parcela->retornarObjeto($_POST['id_parcela']);

        $contaPagar = $this->contasreceberimoveis->retornarObjeto($parcela->id_conta_pagar);

        $contaCorrente = $this->contacorrente->retornarObjeto($_POST['id_conta_corrente']);

        /// Adiciona na tabela extrato
        $extrato['transacao'] = 'Recebimento de Parcela (Contas a Receber)';
        $extrato['id_conta_corrente'] = $contaCorrente->id;
        $extrato['tipo'] = 'C';
		...
		if(!Pagamento::efetuar($_POST['valor_total'], $_POST['id_conta_corrente'])){
			...
			$this->redirect('contasreceberimoveis/pagarparcela/'.$_POST['id_parcela']);
		};
		...
		$this->redirect('contasreceberimoveis/parcelas/'.$parcela->id_conta_pagar);
	}
