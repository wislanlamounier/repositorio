...
class Pagamento extends Model{
	...
	function __construct(){
		$this->tabela = 'conta_receber_pagamento_imoveis';
	}
	...
	function carregarView($id_pagamento){
		$sql = 'select (select count(sub.id_conta_pagar) from conta_receber_parcela_imoveis sub where sub.id_conta_pagar = parcela.id_conta_pagar) as quantidade,
			   		    pagamento.*, corrente.*, f.nome nome_forma_pagamento, pes.nome nome_conta, parcela.numero_parcela from conta_receber_pagamento_imoveis pagamento
				inner join conta_receber_parcela_imoveis parcela on parcela.id = pagamento.id_parcela
				inner join conta_receber_imoveis conta on conta.id = parcela.id_conta_pagar
