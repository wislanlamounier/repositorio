				...
				<table class="table table-striped dataTables_full table table-striped" id="dt_gal">
					...
					<tbody>
						<?php foreach($colecao->lista as $item){ ?>
							<tr>
								<td>
									<?php echo $this->botaoIcone('contacontabil/subconta/'.$item->id_conta.'/'.$item->id,'icon-pencil'); ?> 
									<?php echo $this->botaoIcone('contacontabil/subcontaexcluir/'.$item->id,'icon-remove'); ?>
								</td>
